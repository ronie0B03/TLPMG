$(document).ready(() => 
{
    ...
    $("#table_items").on('click', '._btn_add_item', function()
        {
            const tbody    = $("#table_items > tbody");
            const id       = jQuery.trim($("#table_items ._input_id").val());
            const item     = jQuery.trim($("#table_items ._select_item").find('option:selected').text());
            const quantity = jQuery.trim($("#table_items ._input_quantity").val());
            const price    = jQuery.trim($("#table_items ._input_price").val());
            const subtotal = jQuery.trim($("#table_items ._input_subtotal").val());
            ...
            const row      =    `<tr>
                                    ...
                                    <td>
                                        <input type="text" class="form-control" name="items[]" value="${item}" readonly>
                                    </td>
                                    <td>
                                        <input type="number" class="form-control" name="item_quantity[]" value="${quantity}" placeholder = "0" readonly>
                                    </td>
            ...
            $("#table_items ._select_item").val(''); 
            $("#table_items ._input_price").val(0); 
            $("#table_items ._input_quantity").val(0); 
            $("#table_items ._input_subtotal").val(0); 
    ...
    $("#table_items").on('click', '._btn_remove_item', function()
        {
            const subtotal = $(this).parent().parent().parent().find('._item_subtotal').val(); 
            const total    = $('#total_amount').html();
            const amount   = (total - subtotal);
    ...
    $("#table_items").on('input', '._input_quantity', function()
        {
            const quantity = $(this).val();
            const price    = $("#table_items ._input_price").val();
            const subTotal = $("#table_items ._input_subtotal");
    ...
    $("#table_items").on('change', '._select_item', function()
        {
